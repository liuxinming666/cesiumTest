<template>
    <div id="cesiumContainer">
        <div class="tips" v-show="bShowTip" ref="tip"></div>
    </div>
</template>
        ...
        methods:{
            ...
            initChart:function () {
                ...
                for(var i=0;i< pointList.length;i++){
                    var point=g_viewer.entities.add({
                        id:"point"+i,
                        name:pointList[i].name+":"+pointList[i].value[2],
                        description:"地点："+pointList[i].name+"<br/>值:"+pointList[i].value[2],
                ...
                //可以通过cesium事件在这些点上添加额外的点击、悬浮等事件
                var tip = this.$refs.tip;//$("#tip");

                /*var handler = new Cesium.ScreenSpaceEventHandler(g_viewer.scene.canvas);
                handler.setInputAction(function (e) {
                    var pick = g_viewer.scene.pick(e.endPosition);
                    if (Cesium.defined(pick)&&(pick.id._id.indexOf("point")!=-1)) {
                        var cartesian = g_viewer.camera.pickEllipsoid(e.endPosition, g_viewer.scene.globe.ellipsoid);
                        var canvasPosition =g_viewer.scene.cartesianToCanvasCoordinates(cartesian,new Cesium.Cartesian2());//cartesianToCanvasCoordinates 笛卡尔坐标（3维度）到画布坐标
                        ...
                        tip.show().html(pick.id._description.getValue());
                    }else{
                        //tip.hide();
                        this.bShowTip = false;
                    }
