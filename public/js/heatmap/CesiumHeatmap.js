;var CesiumHeatmap = (function(Cesium,h337){
    function CH(chviewer,geojsonUrl){
        ...
        if(geojsonUrl){
            ...
            //this.loadHeatMapData(geojsonUrl);
        }
    }

    CH.prototype.loadHeatMapData = function(heatMapData){
        var lonmin=1000;
        var lonmax=-1000;
        var latmin=1000;
        var latmax=-1000;
        for(let i=0;i<heatMapData.length;i++){
            var lon = heatMapData[i].x;
            var lat = heatMapData[i].y;
            lonmin = lon<lonmin?lon:lonmin;
            latmin = lat<latmin?lat:latmin;
            lonmax = lon>lonmax?lon:lonmax;
            latmax = lat>latmax?lat:latmax;
        }
        /*heatMapData.forEach(function(heatData){
            var lon = heatData.x;
            var lat = heatData.y;
            lonmin = lon<lonmin?lon:lonmin;
            latmin = lat<latmin?lat:latmin;
        ...
        var points = [];
        for(let i = 0;i < heatMapData.length;i++){
            var x = (heatMapData[i].x-extent.xMin)/(extent.xMax-extent.xMin)*width;
            var y = (-(heatMapData[i].y-extent.yMin)/(extent.yMax-extent.yMin)+1)*height;
            var point = {
                x: parseFloat(x),
                y: parseFloat(y),
                value: parseFloat(heatMapData[i].value)
            };
            points.push(point);
        }
        debugger;
        /*heatMapData.forEach(function(heatData){
            var x = (heatData.x-extent.xMin)/(extent.xMax-extent.xMin)*width;
            var y = (-(heatData.y-extent.yMin)/(extent.yMax-extent.yMin)+1)*height;
            var point = {
                x: x,
                y: y,
                value: 0.02
